...
if [ -z "$KEYFILE" ]
then
    /var/lib/text-pair/textpair_env/bin/gunicorn --timeout 600 -k uvicorn.workers.UvicornWorker -b 0.0.0.0:$PORT -w 4 --access-logfile=/var/lib/text-pair/api_server/access.log --error-logfile=/var/lib/text-pair/api_server/error.log --chdir /var/lib/text-pair/api/ text_pair:app
else
